...
* `ADDF { e1, e2 }` – положить на стек результат операции сложения
  чисел с плавающей точкой e2 + e1.
* `SUBF { e1, e2 }` – положить на стек результат операции вычитания
  чисел с плавающей точкой e2 - e1.
* `MULF { e1, e2 }` – положить на стек результат операции умножения
  чисел с плавающей точкой e2 * e1.
* `DIVF { e1, e2 }` – положить на стек результат операции деления
  чисел с плавающей точкой e2 / e1.
* `FTOI { e1 }` – преобразование числа с плавающей точкой в целочисленное.
